...
class User{
	...
	public function __construct(){
		try{
		$this->db=new PDO("mysql:host=localhost;dbname=food_store;charset=UTF8","root","");
	...
	public function addOrder($order){
		...
		if($conf){
			session_unset();
			echo "<script> alert('Successfully add an order'); window.location.replace('index.php?page=cart');</script>";
		}
		else {
				echo "<script> alert('We are working to solve this problem')</script>";
...
$mail->addAddress("bdfoods@gmail.com");                 
...
if($q->execute([$obj->first_name,$obj->last_name,$obj->reservation_date,$obj->people_number,$obj->phone] )) {
			echo "<script>alert('Successful table reservation.')</script>";

		} else{
			echo "<script>alert('Problem with table reservation.')</script>";
		}
	}
